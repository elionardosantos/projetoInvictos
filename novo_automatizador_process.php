...
<body>
    <?php
        ...
        function verificaReferenciaBD($codigoProduto){
            require('config/connection.php');
            $sql = "SELECT `codigo` FROM `motores` WHERE `codigo` = :codigo AND `deleted` = :deleted";
        ...
        function novoProduto(){
            ...
            try {
                require('config/connection.php');
                $sql = "INSERT INTO `motores`(`id`,`codigo`,`titulo`,`multiplicador`,`peso_minimo_porta`,`peso_maximo_porta`,`ativo`,`selecionado`,`deleted`,`created_by`,`created_at`)
                    VALUES (:id, :codigo, :titulo, :multiplicador, :peso_minimo_porta, :peso_maximo_porta, :ativo, :selecionado, :deleted, :created_by, :created_at)";
